...
def bank_transaction_auto_match(doc, event=None): 
        ...
        if doc.reference_number is not None:
            ...
            if len(doc.reference_number) > 10: 
                ...
                if invoices is not None: 
                    ...
                    threshold = frappe.db.get_value("QR Code Settings", doc.company, "threshold_payment_difference")
    ...
    # If the Payment is unreconciled, we get the user from QR Code Settings and assign it to the dataset
    if doc.status == "Unreconciled": 
        assign_unreconciled_transactions_to = frappe.db.get_value("QR Code Settings", doc.company, "assign_unreconciled_transactions_to")
