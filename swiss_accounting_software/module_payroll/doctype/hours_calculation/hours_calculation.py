# Copyright (c) 2025, Adriatik Mehmeti and contributors
...
class HoursCalculation(Document):
...
def calculate_hours_for_employee(doc):
	...
	calculation_setting = frappe.get_doc("Payroll Calculation Settings", calculation_settings[0]["name"])
...
def enqueue_hours_calculation():
    ...
    frappe.enqueue(
		method="swiss_accounting_software.module_payroll.doctype.hours_calculation.hours_calculation.run_hours_job",
		queue="long",
	...
	for setting in calculation_settings:
		...
		for employee in employees:
			...
			if not existing:
				...
				doc = frappe.new_doc("Hours Calculation")
				doc.status = "Open"
