  ...
  refresh: (frm) => {
    if (frm.doc.docstatus == 1) {
      frm.add_custom_button("Re Export", function () {
        ...
        fetch(
          encodeURI(
            `/api/method/swiss_accounting_software.module_utils.utils.reset_account?start_date=${start_date}&end_date=${end_date}&company=${company}&doctype=${doctype}&docname=${docname}`
          )
        )
          .then((data) => data.json())
